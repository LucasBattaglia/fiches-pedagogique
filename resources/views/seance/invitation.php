            ...
            <div class="card__body" style="padding:40px 32px">
                ...
                <?php if ($invitation): ?>
                    <p class="text-muted" style="margin-bottom:24px">
                        ...
                        vous invite à collaborer sur la séance :
                    </p>
                    <div style="background:var(--bleu-pale);border:1px solid var(--bleu-clair);border-radius:var(--rayon-lg);padding:16px 20px;margin-bottom:28px">
                        <div style="font-family:var(--font-titre);font-size:1.1rem;color:var(--bleu)">
                            <?= htmlspecialchars($invitation['seance_titre']) ?>
                        </div>
                        <div class="text-sm text-muted" style="margin-top:4px">Rôle : Éditeur</div>
                    </div>

                    <?php if ($isLogged): ?>
                        <form action="<?= $base ?>/seance/invitation/<?= htmlspecialchars($token) ?>/accepter" method="post">
                            <button type="submit" class="btn btn--primary" style="width:100%;justify-content:center;margin-bottom:12px">
                                ✅ Accepter et rejoindre
                            </button>
                        </form>
                        <a href="<?= $base ?>/dashboard" class="btn btn--ghost" style="width:100%;justify-content:center">
                            Refuser
                        </a>
                    <?php else: ?>
                        ...
                        <a href="<?= $base ?>/auth/login?redirect=<?= urlencode($base.'/seance/invitation/'.$token) ?>"
                           class="btn btn--primary" style="width:100%;justify-content:center;margin-bottom:12px">
                            Se connecter pour accepter
                        </a>
                        <a href="<?= $base ?>/auth/register?redirect=<?= urlencode($base.'/seance/invitation/'.$token) ?>"
                ...
                <?php else: ?>
                    ...
                    <a href="<?= $base ?>/dashboard" class="btn btn--ghost" style="margin-top:16px">
